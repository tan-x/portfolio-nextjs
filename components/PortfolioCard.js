...
export default function PortfolioCard(props) {
  ...
  const techArray = [
    ...
    },
    {
      type: 'sequelize',
      icon: (
        <OverlayTrigger placement='bottom' overlay={<Tooltip id="sequelize-icon">Sequelize</Tooltip>}>
            ...
            <Card.Body className='port-card-body'>
              <div className='port-body-hidden'>
                  ...
                  >
                    {props.tech &&
                      props.tech.map((item, index) => {
                        ...
                        if (foundIndex >= 0) {
                          return (
                            <div key={index}>{techArray[foundIndex].icon}</div>
                          );
                        }
                      })}
                  </div>
                ...
                <Fade right>
                  <img
                    onMouseEnter={() =>
                      setHover({ right: '-7%', filter: 'opacity(100%)' })
                    }
                    onMouseLeave={() =>
                      setHover({ right: '-10%', filter: 'opacity(80%)' })
                    }
                    style={{
                      width: '60%',
                      position: 'absolute',
